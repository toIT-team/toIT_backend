...
```
알림 처리량 개선 — 동시 발송 구조 전환
- 한 건씩 순차 발송하던 스케줄러를 스레드 4개 동시 발송으로 바꿔
  분당 150건 → 943건 (한 건 처리 시간은 316ms → 321ms 로 유지)
- 발송 1건의 85%가 FCM 왕복 대기임을 구간 분해로 확인하고, 스레드 수는
  N=1·2·4 실측(선형 3.8배)과 커넥션 풀 점유 0.53/10 으로 결정
- 시한을 넘긴 알림을 EXPIRED 로 기록해 유실률 측정을 가능하게 함.
  밀림 재현 테스트에서 유실 30건 → 0건
```
...
```
[문제 상황]
- 앞 글에서 재처리를 넣어 유실을 막았으나, 밀린 알림이 한 실행에 몰리면
  비우는 데 10분이 넘어 그동안 도착한 새 알림이 사라짐
- 2,500건 재현 테스트에서 12.8분이 걸렸고, 그사이 도착한 30건이 유실
- 유실된 건은 PENDING 인 채 조회에서 빠지기만 해 몇 건인지 셀 수도 없었음

[원인 분석]
- 2 vCPU 에서 CPU 가 10%를 못 넘기는데 처리량도 안 오르는 상태
- 로그 구간 분해로 한 건 339ms 중 289ms(85%)가 FCM 응답 대기임을 확인
- CPU 가 낮은 것과 처리량이 낮은 것이 같은 원인 — 기다리는 동안 CPU 에도
  안 잡히고 다음 알림도 못 집음

[해결 과정]
- ExecutorService 로 동시 발송. 스레드 수는 실기기 3대로 N=1·2·4 를 실측해
  선형(1.00 → 1.94 → 3.82)임을 확인하고, 느린 한 건이 5초를 붙잡아도 목표를
  지키는 최소값으로 4 를 선택
- 외부 호출을 트랜잭션에 넣으면 FCM 은 롤백이 안 되고 커넥션을 321ms 쥐게 되어,
  원자성 대신 멱등키 + 상태 기반 회수(at-least-once)로 처리
- 조회 정렬을 DESC → ASC 로 바꿔 마감이 가까운 것부터 발송 (EDF)
- 시한을 넘긴 알림을 벌크 UPDATE 한 문장으로 EXPIRED 표시 (200건 0.2초)

[측정]
- 커넥션 풀 점유를 게이지가 아닌 usage timer 로 재서 평균 0.53/10 확인.
  한 건이 커넥션을 쥐는 시간이 40ms 로 일정해 풀이 병목이 아님을 확인
- 개선 전과 같은 시나리오(2,500건 + 분당 10건 유입)를 다시 돌려 검증

[결과] 분당 150건 → 943건, 2,500건 드레인 12.8분 → 2.7분, 유실 30건 → 0건
```
...
```
계산이 틀린 것을 그대로 적음   N=4 CPU 를 50%로 예측했으나 실제 34%,
                             그마저 스크랩 1회라 못 믿는 값이었음을 밝힘
지표를 바꿔 다시 잼            커넥션 active 게이지가 계속 0 으로 나와,
                             시간을 적분하는 usage timer 로 갈아끼움
안 고치기로 한 것을 적어둠     만료 알림을 알림함에 넣을지 검토했으나
                             제품 결정과 어긋나 세는 것까지만 함
아직 안 고친 것을 밝힘         @Version 이 없어 발송 중 수정이 덮어써지는 문제.
                             동시성을 넣어 노출이 네 배가 된 것도 같이 적음
근거가 흔들린 것을 인정함      N=8 을 포기한 이유가 FCM 기기 제한이었는데,
                             나중에 그 제한을 넘겨도 거절이 없는 것을 관측함
```
