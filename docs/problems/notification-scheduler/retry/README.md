...
| | 개선 전 | 개선 후 |
|---|---|---|
| **A** 서버 재시작 구간 도달 | **0 / 100** | **100 / 100** |
| **B** 발송 거절 구간 도달 | **0 / 100** | 0 / 100 |
| **B** 의 DB 기록 | `is_sent = true` (보냄) | **`FAILED` · `NO_TOKEN`** |
| 100건 드레인 | — | 38 초 |
...
- **서버가 내려간 구간의 유실이 사라졌습니다.** 지나간 알림도 다음 실행이 집어
  100건 전부 도달했고, 드레인 38초로 정상 발송과 같은 수준이었습니다.
- **발송 거절 구간은 도달 0 그대로입니다.** 토큰이 하나도 없으면 보낼 데가 없어
  재시도하지 않기로 한 것이라 의도한 결과입니다. 대신 `NO_TOKEN` 으로 남아
  **왜 못 갔는지 셀 수 있게** 됐습니다.
- **처리량은 안 변했습니다.** 한계도 165~175건 그대로입니다. 고친 것이 기록과
  조회라 발송 속도를 건드리지 않았고, 시간의 85%가 FCM 왕복이라 애초에 DB 쪽으로는
  움직일 수 없는 값이었습니다.
