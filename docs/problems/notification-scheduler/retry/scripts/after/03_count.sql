...
SELECT
    ...
    SUM(a.attempt_count > 0)            AS 재시도한_건수
FROM schedules_alarm a
JOIN schedules s ON s.schedules_id = a.schedules_id
WHERE s.title LIKE '유실테스트-%';

-- 1-1. 왜 실패했는지
SELECT a.last_error_code AS 오류, COUNT(*) AS 건수
FROM schedules_alarm a
JOIN schedules s ON s.schedules_id = a.schedules_id
WHERE s.title LIKE '유실테스트-%' AND a.last_error_code IS NOT NULL
GROUP BY a.last_error_code;
